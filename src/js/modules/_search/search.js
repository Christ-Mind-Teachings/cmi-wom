...
import {getSearchResult, getSearchResultsList, putSearchResult} from "www/modules/_ajax/searchResults";
...
function makeList(savedSearches) {
  ...
  //save descriptions
  savedSearchDescriptions = {};
  savedSearches.forEach(i => {
    savedSearchDescriptions[i.uniqueId] = {desc:i.desc, name:i.name, count:i.count};
  });
...
function updateSavedSearchList() {
  ...
  if (userInfo) {
    getSearchResultsList(userInfo.userId, "wom")
      .then((response) => {
...
function initSearchModal() {
  ...
  $("#search .show-desc").on("click", function(e) {
    ...
    let ssd = savedSearchDescriptions[id];
  ...
  $(".cmi-search-list").on("click", ".save-modified-search-list", function(e) {
    ...
    let queryResult = storeGet("srchResults");
    let removedItems = [];
    ...
    //update modified list in local store
    storeSet("srchResults", queryResult);
...
async function saveModifiedSearchResults() {
  ...
  try {
    let result = await putSearchResult(userInfo.userId, "wom", queryResult);
